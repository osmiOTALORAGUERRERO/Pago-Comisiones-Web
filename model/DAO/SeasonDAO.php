<?php
  ...
  class SeasonDAO implements InterfaceSeason
  {
    ...
    public function updateSeason($season)
    {
      ...
      $sql = 'UPDATE Seasons SET name_season=:name_season, number_sellers=:number_sellers, porcentage_products=:porcentage_products, active=:active WHERE id_season=:id_season';
    ...
    {
      $dataBase = new DataBaseConection();
      $result = $dataBase -> executeQuery('SELECT * FROM Seasons WHERE name_season IS NOT NULL');
      $seasons = array();
      for ($i=0; $i < count($result); $i++) {
        $season = new Season();
        $season -> setId($result[$i]['id_season']);
    ...
    {
      $dataBase = new DataBaseConection();
      $sql = 'UPDATE Seasons SET active = :active WHERE id_season = :id_season';
      $result = $dataBase -> executeUpdate($sql, array(
        ':id_season'=>$idSeason,
        ':active'=>$active
      ));
    ...
    {
      $dataBase = new DataBaseConection();
      $sql = 'DELETE FROM SeasonBySeller WHERE id_season=:id_season';
      $result = $dataBase -> executeDelete($sql, array(':id_season'=>$idSeason));
      return $result;
    }
  }
